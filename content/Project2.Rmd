...
check %>% ggplot(aes(recodedgpa,cdisc,fill=recodedgpa))+geom_bar(stat="summary")+
  geom_errorbar(stat="summary") +coord_flip()+ylab("cdisc score")+theme(legend.position = "none")
...
{
  hist(rand_distribution, main = "", ylab = "")
...
```
Do mothers or fathers experience more personal discrimination on average?
...
fitbpi %>% ggplot(aes(cdisc_centered, totbpi)) + geom_smooth(method= "lm", aes(color= SCHCOMP), se= F) 
...
surveybpi$bpi_Cat <- ifelse(surveybpi$totbpi > mean(surveybpi$totbpi), "above mean", "below mean")


ggplot(surveybpi,aes(x= logit,fill=bpi_Cat))+geom_density(alpha=.75)
...
ROCplot <- ggplot(surveybpi) + geom_roc(aes(d = totbpi, m = prob), n.cuts = 0)
ROCplot
calc_auc(ROCplot)
...
After running a lasso-regression, the coefficients that are non-zero for predicting whether a person has higher or lower than average bpi score are PART_EDU4 (partner's highest education level is some college),  MMRI (racial identity score), pdisc, cdisc_centered, and recodedgpa. Plugging in these variables into a cross validation improves the results of our model, and it is doing a pretty good job of predicting which bpi category a child falls in. The accuracy is 0.7499390, sensitivity (true positive rate) is 0.5577210, specificity (true negative rate) is 0.8659588, precision (postitive predictive value) is 0.7140565 and the area under the curve is 0.8007965.
...
ROCplot <- ggplot(Income) + geom_roc(aes(d = recodedincome, m = prob), n.cuts = 0)
ROCplot
calc_auc(ROCplot)
